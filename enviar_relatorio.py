...
saida_dir = 'saida'
modelo_nome = (
    f"{datetime.today().strftime('%Y%m%d')}_MONITORAMENTO DE COMPONENTE.xlsx"
)
...
def copiar_para_publico():
    if os.path.exists(relatorio_path):
        try:
            ...
            print(
                f'⚠️ Permissão negada ao copiar o arquivo. Verifique se ele está aberto: {relatorio_path}'
            )
    else:
...
def enviar_email():
    ...
    try:
        ...
        email.To = 'otavio.santos@saude.gov.br'
        email.Subject = (
            f"Relatório Diário - {datetime.today().strftime('%d/%m/%Y')}"
        )
    ...
    for arquivo in os.listdir(pasta):
        ...
        if os.path.isfile(caminho_arquivo):
            try:
                ...
                print(
                    f'⚠️ Arquivo em uso, não foi possível excluir: {caminho_arquivo}'
                )
